...
@db.apps.streamlit("/apps/sales-forecasting", name="Sales forecasting")
def app():
    ...
    option = st.sidebar.selectbox(label="Which statistic do you want to examine?", options=stats)
    df = frames[option]
    ...
    plot_what = st.selectbox(label="Plot even more trends for", options=dG.index)
    ...
    s = dG.style.set_properties(**{"background-color": "#b987ffff"}, subset=slice_)
